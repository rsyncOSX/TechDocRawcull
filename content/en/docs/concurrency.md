...
The app uses Swift 6 with default main-actor isolation: presentation state begins
on `@MainActor`, shared mutable background state lives in actors, and CPU,
decode, or file work leaves inherited UI isolation explicitly.
...
`RawCullApp` is the composition root. Its initializer creates
`RawCullAIIntegration`, then creates the one `RawCullViewModel` and injects
the selected similarity, semantic-search, and Deep Review services. It also
creates `RawCullAISettingsModel` with callbacks that return capability changes
...
and selection, operation tasks and generations, feature models, progress, and
the active catalog security scope. Child main-actor models such as
`SharpnessScoringModel`, `SimilarityScoringModel`, `CullingModel`, and
`DeepAIReviewFeature` own narrower workflow state. Background actors own the
mutable state required to perform those workflows.

```mermaid
flowchart LR
    APP["RawCullApp<br/>composition root"] --> VM["@MainActor RawCullViewModel<br/>catalog and workflow owner"]
    APP --> AIS["@MainActor RawCullAISettingsModel"]
    VM --> VIEW["RawCullMainView and SwiftUI presentation"]
    VM --> MODELS["Main-actor feature models"]
    MODELS --> ACTORS["Background actors and package services"]
...
flowchart TB
    ...
    subgraph BURST["Burst indexing"]
        B1["MainActor generation and catalog snapshot"] --> B2["bounded PhotoAIKit indexing"]
        B2 --> B3["provider and artifact-store actors"]
        B3 --> B4["grouping and ranking work"]
        B4 --> B5["publish only if generation + catalog still match"]
        B5 --> B6["actor-backed cache commit"]
    end
...
|---|---|
| `RawCullViewModel` | Active catalog, selection, security scope, catalog/preload/export/hydration tasks, burst generation, presentation state |
...
| `DeepAIReviewFeature` | Review generation, progress, recommendation, and cancellation |
| `RawCullAISettingsModel` | Capability-refresh generation and model-download presentation |
...
Actor serialization does not make independent work parallel. Where file work
can overlap, owners use a bounded task group and keep only a sliding window of
children active.
...
and PhotoAnalysisKit's Core Image/Vision worker. PhotoAnalysisKit 1.2.0 uses an
explicit concurrent child and a cancellation handler; it does not use a
detached task for the focus engine.
...
blocking image representations. The owner still awaits or otherwise controls
the result, and propagates cancellation where required.
...
1. `ThumbnailPreloadGate` blocks only grid misses whose parent directory is
   the catalog currently being preloaded. Other catalogs, preview requests, AI
...
3. `RequestThumbnail` coalesces only an exact `ThumbnailCacheKey`. Each
   waiter owns its continuation. Cancelling one waiter preserves the shared
   producer; cancelling the final waiter removes the table entry and cancels
   the producer.
...
Cancellation is cooperative and is not enough by itself. A callback or
framework operation may complete after cancellation, so owners pair task
cancellation with identity checks.
...
Every successful security-scope start has one owner that records the URL and
