...
date = "2026-07-18"
description = "A detailed learning guide to PhotoAIKit's products, dependency boundaries, contracts, backends, workflows, storage, concurrency, and model identity."
tags = ["ai", "swift-package", "clip", "sam3", "architecture"]
...
### 3.4 Make Persisted Artifacts Self-Describing
...
### 3.5 Treat Model Identity As Data
...
`CoreAICLIPProvider` is an actor conforming to embedding generation, artifact generation, and artifact comparison protocols.

Its responsibilities are deliberately backend-specific:

- validate the supplied CLIP bundle;
- lazily load and cache the Core AI model function;
- validate expected tensor names and shapes;
- create the image, token, and attention-mask NDArrays;
- resize into sRGB and apply CLIP channel normalization;
- run the model, flatten the output, and L2-normalize it through `ImageEmbedding`;
...
`Tests/PhotoAIKitTests/` imports the products through their public APIs rather than using `@testable import`. Its tests cover architectural promises such as:
...
The package includes tools under `Tools/` for exporting CLIP and SAM 3 assets, selecting a SAM 3 asset, and generating fingerprints. Output paths are explicit.
...
6. Reuse `SimilarityArtifactIndexer` and inject the host's decoder.
7. Decide explicitly whether fallback is none, per-item, or whole-batch.
8. Add public-API tests with fake sources and model-free inputs where possible.
9. Let the host choose model URLs, cache locations, settings, and product policy.
...
| CLIP backend | `Sources/CoreAICLIPBackend/CoreAICLIPProvider.swift` |
...
Next, follow these abstractions into the host application in [How RawCull Enables and Uses CLIP](../../ai/clip-in-rawcull/).
