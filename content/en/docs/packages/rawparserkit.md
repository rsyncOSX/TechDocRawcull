...
3. EXIF exposure time, aperture, focal length, ISO, exposure bias, lens, and
   dimensions;
...
2. Implement vendor-specific thumbnail, preview, focus, compression, and
   size-class behavior.
3. Put TIFF or MakerNote byte rules in a dedicated parser, including explicit
   offset bases and bounds checks.
4. Return the common normalized focus-location string at the public boundary.
5. Add ImageIO behavior first and a binary embedded-JPEG fallback when the
...
8. Add synthetic binary tests for endian, offsets, missing tags, corrupt
...
[How PhotoAnalysisKit Is Constructed](../photoanalysiskit/), or return to the
[package overview](/docs/packages/).
